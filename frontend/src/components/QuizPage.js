...
const QuizPage = ({ user, logout }) => {
  ...
  const questions = [
    ...
    {
      question: "Как следует относиться к письмам, приходящим на корпоративную почту с внешних адресов?",
      options: [
        "Открывать сразу, так как это важная информация",
        "Относиться с повышенной настороженностью и проверять отправителя",
        "Пересылать коллегам для ознакомления", 
        "Игнорировать все внешние письма"
      ],
      correct: 1,
      explanation: "Письма с внешних адресов на корпоративную почту требуют повышенного внимания - всегда проверяйте отправителя перед любыми действиями."
    },
    {
